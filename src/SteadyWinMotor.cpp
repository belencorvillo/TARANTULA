...
void MWSetAxisState(uint8_t busId, uint8_t nodeId, MW_MOTER_STATE state) {
    if (busId >= MAX_BUS_NUM || nodeId >= MAX_MOTOR_NUM_PER_BUS)
        return;
    MotorSender sender = motors[busId][nodeId].sender; //sender es la función lambda de envío de los motores, gestionada por la clase MotorController
    if (sender == 0) return;

    uint8_t txBuff[8] = { 0 }; //creamos un vector de 8 bytes
    memcpy(txBuff,&state, 4); //copiamos el nuevo estado (4 bytes) dentro de nuestro vector de 8 bytes
    sender(busId, ((uint16_t)nodeId << 5) | MW_SET_AXIS_STATE_CMD, txBuff, sizeof(txBuff)); //usamos el sender para establecer el nuevo estado
}
...
MW_MIT_CTRL MWMitControl(uint8_t busId, uint8_t nodeId, MW_MIT_CTRL* mit) {
    ...
    uint8_t txBuff[8] = { 0 };
    //hacemos la conversión de double a int16
    ...
    //mapeo de los valores en 8 bytes
    txBuff[0] = pos_int >> 8;
    ...
    txBuff[7] = t_int & 0xFF;
    //mandamos las consignas MIT
    sender(busId, ((uint16_t)nodeId << 5) | MW_MIT_CONTROL_CMD, txBuff, sizeof(txBuff));

    //al haber mandado los datos la función MWReceiver ha ejecutado la función MWMitControlRec que sobreescribe
    //los doubles motorMIT.pos, motorMIT.vel y motorMIT.torque
    //devolver la estructura motorMIT con sus datos actualizados:
...
MW_ENCODER_ESTIMATES MWGetEncoderEstimates(uint8_t busId, uint8_t nodeId) {
    ...
    uint8_t txBuff[8] = { 0 };
    //le mandamos el comando de lectura del encoder sin trama de datos
    sender(busId, ((uint16_t)nodeId << 5) | MW_GET_ENCODER_ESTIMATES_CMD, txBuff, sizeof(txBuff));

    return motors[busId][nodeId].motorData->encoderEstimates; //devuelve velocidad y posición leídas por el encoder
}
...
void MWReceiver(uint8_t busId, uint32_t canId, uint8_t* data) {
     ...
     // Escudo antirruido: si el mensaje tiene un ID mayor de 2047, es ruido o un error del cable
    if (canId > 2047) return;

    // Si el mensaje no tiene datos, lo ignoramos
    ...
    // Si el motor no existe en nuestra memoria, lo ignoramos
    if (motor->motorData == nullptr) return;

    //ejecutamos la función correspondiente según el ID del comando:
