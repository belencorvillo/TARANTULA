...
#define MAX_MOTOR_NUM_PER_BUS 128
    ...
    typedef struct {
        struct {
            uint32_t axisError;                 //（odrv0.axis0.error）
            uint8_t motorErrorFlag;             //（odrv0.axis0.motor.error 是否为0）
            uint8_t encoderErrorFlag;           // (odrv0.axis0.encoder.error 是否为 0）
            uint8_t controllerErrorFlag;        //（odrv0.axis0.controller.error 是否为0）
            uint8_t ErrorFlag;                  //（odrv0.error 是否为 0）
        } ErrorStatus;                          
    ...
    typedef struct {
        float encoderPosEstimate;               
        float encoderVelEstimate;               
    ...
    typedef void (*MotorSender)(uint8_t busId, uint32_t canId, uint8_t* data, uint8_t dataSize);
